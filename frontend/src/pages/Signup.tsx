...
import { User, Phone, MapPin, ChevronDown, Globe, Mail, Lock, Camera } from 'lucide-react';
    ...
    <div className="flex h-screen w-full bg-white overflow-hidden font-poppins">
      ...
      <div className="w-full lg:w-[45%] flex flex-col p-8 lg:p-12 overflow-y-auto CustomScrollbar">
        ...
        <div className="max-w-md mx-auto w-full flex-1 flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Profile Photo (Optional)</label>
              <div className="relative group">
                <Camera className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-300 group-focus-within:text-brand-green transition-colors" />
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => setImageFile(e.target.files?.[0] || null)}
                  className="w-full h-12 pl-12 pr-4 bg-gray-50 border border-transparent rounded-2xl text-sm font-medium focus:bg-white focus:border-brand-green/20 focus:ring-4 focus:ring-brand-green/5 outline-none transition-all text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-gray-100 file:text-gray-700 hover:file:bg-gray-200 cursor-pointer"
                />
              </div>
            </div>
      ...
      <div className="hidden lg:flex lg:w-[55%] relative overflow-hidden">
        ...
        <div className="absolute top-12 right-12 flex items-center space-x-4">
          <span className="text-[10px] font-bold uppercase tracking-widest text-white/60">Already have an account?</span>
