...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const newPost = { id: makeId(), authorId: req.user.id, authorName: req.user.name, title, content, timestamp: new Date().toISOString(), replies: [] };
    if (await isMongoEnabled()) {
...
router.post('/:id/replies', requireAuth, async (req, res) => {
  try {
    ...
    const newReply = { id: makeId(), authorId: req.user.id, authorName: req.user.name, content, timestamp: new Date().toISOString() };
    if (await isMongoEnabled()) {
